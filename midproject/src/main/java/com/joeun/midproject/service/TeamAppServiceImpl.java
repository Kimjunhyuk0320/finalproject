...
public class TeamAppServiceImpl implements TeamAppService {
  ...
  public int denied(TeamApp teamApp) {
    ...
    int teamNo = teamAppMapper.read(teamApp).getTeamNo();
    teamApp.setTeamNo(teamNo);
    Team tempTeam = new Team();
    tempTeam.setTeamNo(teamNo);

    Team team = teamMapper.read(tempTeam);
    System.out.println("team정보입니다." + team);
    if (team.getRecStatus() == team.getCapacity()) {
      System.out.println("teamApp정보입니다." + teamApp);
  ...
  public int confirmed(TeamApp teamApp) throws Exception {
    ...
    if (team.getRecStatus() == team.getCapacity()) {
      ...
      teamApp.setTeamNo(teamNo);
      System.out.println(teamApp);
      deniedAllResult = teamAppMapper.deniedAll(teamApp2);
