...
class RentalStateScreen extends StatefulWidget {
  const RentalStateScreen({super.key});

  @override
  State<RentalStateScreen> createState() => _RentalStateScreenState();
}

class _RentalStateScreenState extends State<RentalStateScreen> {
//렌탈 리스트 state
  List _rentalList = [];
  ...
  Future getTeamAppList(user) async {
    var teamListURL =
        'http://13.209.77.161/api/booking/rr?page=${_page}&rows=${_rows}&username=${user.userInfo['username']}';
    ...
    if (teamListResponse.statusCode == 200) {
      ...
      setState(() {
        _page++;
        _rentalList.addAll(tempTeamList);
      });
                  ...
                  Container(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        //요청
                        final String url =
                            'http://13.209.77.161/api/booking/accept';
                        final parsedUri = Uri.parse(url);
                        final headers = {
                          'Content-Type': 'application/json',
                        };
                        final body = {
                          'brNo': _rentalList[index]['brNo'],
                        };
                        ...
                        if (result.statusCode == 200) {
                          print('대관신청 승인 처리가 완료되었습니다.');
                          setState(() {
                            _rentalList[index]['approvalStatus'] = 1;
                          });
                        } else {
                          print('대관신청 승인 처리에 실패하였습니다.');
                        }
                  ...
                  Container(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        //요청
                        final String url =
                            'http://13.209.77.161/api/booking/denied';
                        final parsedUri = Uri.parse(url);
                        final headers = {
                          'Content-Type': 'application/json',
                        };
                        ...
                        if (result.statusCode == 200) {
                          print('대관신청 거절 처리가 완료되었습니다.');
                          setState(() {
                            _rentalList[index]['approvalStatus'] = 2;
                          });
                        } else {
                          print('대관신청 승인 처리에 실패하였습니다.');
                        }
                  ...
                  Container(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        //요청
                        final String url =
                            'http://13.209.77.161/api/booking/confirm';
                        final parsedUri = Uri.parse(url);
                        final headers = {
                          'Content-Type': 'application/json',
                        };
                        ...
                        if (result.statusCode == 200) {
                          print('대관 확정 처리가 완료되었습니다.');
                          setState(() {
                            _rentalList[index]['depositStatus'] = 1;
                          });
                        } else {
                          print('대관 확정 처리에 실패하였습니다.');
                        }
                        Navigator.pop(context);
                      },
                      child: Text(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(
                    image: AssetImage('assets/images/defaultRentalImg.jpeg'),
                    fit: BoxFit.cover,
              ...
              Container(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    if (index < _rentalList.length) {
                      final item = _rentalList[index];
                      return GestureDetector(
                        ...
                        onTap: () async {
                          var parsedUrl = Uri.parse(
                              'http://13.209.77.161/api/fr/${_rentalList[index]['frNo']}');
                          var response = await http.get(parsedUrl);
                          var decodedResponse =
                              json.decode(utf8.decode(response.bodyBytes));
                          var rental = Rental(
                            boardNo: decodedResponse['frNo'],
                            title: decodedResponse['title'],
                            writer: decodedResponse['writer'],
                            ...
                            liveDate: decodedResponse['liveDate'],
                            updDate: decodedResponse['updDate'],
                            phone: decodedResponse['phone'],
                            account: decodedResponse['account'],
                            views: decodedResponse['views'],
                            confirmed: decodedResponse['confirmed'],
                            thumbnail: decodedResponse['thumbnail']['fileNo'],
                            isCaching: false,
                          );
                          Navigator.pushNamed(
                            context,
                                          ...
                                          child: ListTile(
                                            title: Row(
                                              ...
                                              children: [
                                                Container(
                                                  child: Text(
                                                    '${item['frTitle'].length > 9 ? item['frTitle'].substring(0, 9) + '...' : item['frTitle']}',
                                                    style: TextStyle(
                                                      fontSize: 18.0,
                                                      fontWeight:
                                                          FontWeight.bold,
                  ...
                  itemCount: _rentalList.length + 1,
                ),
              ),
            ],
          ),
