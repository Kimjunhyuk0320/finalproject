...
class MyRentalAppScreen extends StatefulWidget {
  const MyRentalAppScreen({super.key});

  @override
  State<MyRentalAppScreen> createState() => _MyRentalAppScreenState();
}

class _MyRentalAppScreenState extends State<MyRentalAppScreen> {
//팀 리스트 state
  List _teamList = [];
  ...
  Future getTeamAppList(user) async {
    var teamListURL =
        'http://10.0.2.2:8080/api/booking/rreq?page=${_page}&rows=${_rows}&username=${user.userInfo['username']}';
                  ...
                  Container(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        //요청
                        final String url =
                            'http://10.0.2.2:8080/api/booking/${_teamList[index]['brNo']}';
                        final parsedUri = Uri.parse(url);
                      ...
                      return GestureDetector(
                        ...
                        onTap: () async {
                          final result = await http.get(Uri.parse(
                              'http://10.0.2.2:8080/api/fr/${item['frNo']}'));
                          Map decodedResult =
                              await json.decode(utf8.decode(result.bodyBytes));
                          var rental = Rental(
                            boardNo: decodedResult['frNo'],
                            title: decodedResult['title'],
                            writer: decodedResult['writer'],
                            username: decodedResult['username'],
                            content: decodedResult['content'],
                            price: decodedResult['price'],
                            location: decodedResult['location'],
                            address: decodedResult['address'],
                            liveDate: decodedResult['liveDate'],
                            updDate: decodedResult['updDate'],
                            phone: decodedResult['phone'],
                            account: decodedResult['account'],
                            views: decodedResult['views'],
                            confirmed: decodedResult['confirmed'],
                            thumbnail: decodedResult['thumbnail']['fileNo'],
                            isCaching: false,
                          );
                          Navigator.pushNamed(
                            context,
