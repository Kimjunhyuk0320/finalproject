...
class _CommentScreenState extends State<CommentScreen> {
  ...
  insert(item.boardNo ?? 0, parentTable, user['writer']!, inputValue, user['username']!, user['profileNo']!);
  ...
  Future<void> insert(int parentNo, String parentTable, String writer, String content, String username, String profileNo) async {
  print('insert...');
  final url = Uri.parse('http://10.0.2.2:8080/api/comment');
  ...
  // 댓글 수정
 Future<void> updateComment(int commentNo, String content, String writer) async{
...
Future<void> deleteComment(int commentNo) async {
  ...
  final url = Uri.parse('http://10.0.2.2:8080/api/comment/$commentNo');
  ...
  Future fetch() async {
    print('fetch...');
    final url = Uri.parse(
        'http://10.0.2.2:8080/api/comment?parentTable=${parentTable}&parentNo=${item.boardNo}&page=${_page}');
                  ...
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(
                        ...
                        borderSide: BorderSide(color: Colors.grey),  // 연하게 설정한 테두리 색상
                      ),
                      focusedBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(10.0)),
                        borderSide: BorderSide(color: Colors.grey),  // 포커스가 있을 때의 테두리 색상
                      ),
                      enabledBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(10.0)),
                        borderSide: BorderSide(color: Colors.grey),  // 활성화되어 있을 때의 테두리 색상
                      ),
                      errorBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(10.0)),
                        borderSide: BorderSide(color: Colors.red),  // 에러 상태일 때의 테두리 색상
                      ),
                      focusedErrorBorder: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(10.0)),
                        borderSide: BorderSide(color: Colors.red),  // 포커스가 있고 에러 상태일 때의 테두리 색상
                      ),
                    ),
                    validator: (value) {
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            SizedBox(width: 10,),
                            Text(
                              DateFormat('yyyy/MM/dd HH:mm:ss').format(DateTime.parse(item.updDate)),
                              style: TextStyle(
                               color: Colors.grey
                              )
                            ),
                        ...
                        (user['writer'] == item.writer && editingComment != item.commentNo)? 
                            Row(
                              children: [
                               ElevatedButton(
                                onPressed: () {
                                        ...
                                        child: Container(
                                          ...
                                          child: Padding(
                                            ...
                                            child: Column(
                                              ...
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text('댓글 수정', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                                                SizedBox(height: 20),
                                                TextFormField(
                                                ...
                                                Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    ElevatedButton(
                                                      onPressed: () {
                                                        // 댓글 수정 처리
                                                        updateComment(item.commentNo, updatedContent, item.writer);
                                                        Navigator.of(context).pop();
                                                      },
                                                      style: ElevatedButton.styleFrom(
                                                        fixedSize: Size(80, 40),
